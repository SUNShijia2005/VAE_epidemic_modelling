"""
Hybrid VAE：decoder 里装 SIR（可微机理模型）+ 一个 NN 残差
================================================================================
对齐与老师讨论的方向。和现有 vae_full.py 是**方向相反**的两套架构：

    vae_full.py（旧）  encoder q(z|θ,x) -> decoder 吐 θ        重建的是「参数」
    本档（新）         encoder q(β,α,ρ|x) -> decoder 吐 x      重建的是「数据」

关键三点：

  1. **低维潜空间就是参数本身**：latent 分两段
         z_phys (3 维) --一一映射--> (β, α, ρ)    ← 可解释，encoder 输出即后验
         z_res  (d 维)                             ← 黑箱残差潜变量
     先验都是 N(0,1)，参数 = exp(μ + σ·z_phys) 是 lognormal 先验（造数据时
     用同一个先验，所以 ELBO 里的 KL 就是标准解析高斯 KL，无近似）。

  2. **decoder = 两个 model**
         机理项：可微 SIR（Euler 展开，对 β,α 可导）→ 机理发病曲线 λ^SIR_t
         黑箱项：MLP(z_res) → 一条平滑时间修正曲线 f_t
     两种拼法（--residual_mode）：
         obs  : log λ_t = log λ^SIR_t + s·f_t          （修观测层/误设，默认）
         beta : β_t = β·exp(s·f_t) 再喂进 SIR          （修传播率随时间变化）
     s = --nn_scale，设 0 就退化成**纯机理 VAE**（对照组）。

  3. **训练不需要 θ 标签**：loss 只用观测 x。这意味着同一套权重可以直接上真实
     数据（旧架构必须先跑 ~10 小时模拟造 (θ,x) 配对）。

最简 SIR（老师说的「先只要最简单的」）：单一均质人群，无年龄、无家户结构：
    dS/dt = -β S I / N,   dI/dt = β S I / N - α I

观测模型（第二步加入，真实数据必需，也是把后验校准修对的关键）：
    y_t ~ NegBin(mean = ρ · 当日新增感染, dispersion k)
    ρ  = 上报率（当作待推断的 nuisance，一并进 latent 并在报告时边缘化掉）
    k  = 过度离散参数（全局可学参数，非逐样本）
  纯 Poisson + 全人群可观测时，一条曲线上万个事件 -> 真后验窄到千分之一量级，
  摊销误差必然盖不住区间（实测 cover90 ≈ 10%）。加上 ρ 和过度离散后，真后验
  宽度回到合理量级，覆盖率才有意义。

训练目标（--iwae K）：
    K=1 -> 普通 ELBO；K>1 -> IWAE（重要性加权），是收紧摊销 gap、
    让 q 不至于过窄的标准做法。代价是 SIR 要展开 K 倍。

用法（产物默认落在 results_hybrid/）：
  # 合模型基线（16 分钟 CPU，cover90 ≈ 89%）
  python sir_hybrid_vae.py --n_train 16000 --epochs 200 --iwae 5 --patience 30 \
      --out results_hybrid/sir_final2
  # 误设基线：真 β 有干预，靠黑箱补（cover90 ≈ 69%）
  python sir_hybrid_vae.py --n_train 8000 --epochs 150 --iwae 5 --patience 25 \
      --misspec 1 --residual_mode beta --free_bits 0.5 \
      --out results_hybrid/mis_fb
  # 纯机理对照（关掉黑箱）
  python sir_hybrid_vae.py --nn_scale 0 --out results_hybrid/sir_pure
"""
...
# ============================================================================
def make_data(n, seed=0, misspec=0.0):
...
class Encoder(nn.Module):
    """full_cov=True 时 q(z_phys|x) 用满协方差高斯（Cholesky）。

    (β,α,ρ) 的后验沿 R0 方向强相关，对角高斯给出的是「条件方差」而不是「边缘
    方差」，实测比真后验窄 3.5 倍（见 check_posterior_vs_grid.py）。3 维潜空间
    做满协方差只多 3 个自由度，几乎不要钱。
    """
...
class HybridVAE(nn.Module):
    def __init__(self, z_res_dim=4, nn_scale=0.3, residual_mode="obs",
...
def train(model, x_tr, y_tr, x_va, y_va, args, device):
    ...
    for ep in range(1, args.epochs + 1):
        ...
        tag = ""
        if ep >= args.kl_warmup:                          # warmup 期目标在变，不判早停
            if va_loss < best_val:
        ...
        if ep >= args.kl_warmup and since >= args.patience:
...
# ============================================================================
@torch.no_grad()
def posterior_phys(model, x_std, n_draws=500):
...
def main():
    ...
    ap.add_argument("--nn_scale", type=float, default=0.3,
                    help="NN 残差强度；0 = 纯机理 VAE（对照组）")
    ap.add_argument("--residual_mode", choices=["obs", "beta"], default="obs")
    ...
    ap.add_argument("--out", type=str, default="results_hybrid/sir_run",
                    help="输出前缀（含目录，自动创建）")
    args = ap.parse_args()
    ...
    print("  [读法] cover90 目标 90%。post_sd << RMSE => q 过窄或 encoder 有偏；"
          "bias 明显非 0 => 摊销偏差，IWAE 救不了，得加 encoder 容量")
