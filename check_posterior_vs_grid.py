"""Constant-SIR numerical posterior reference.
The grid omits neural correction and uses checkpoint dispersion. It is not the
full hybrid posterior. Check both resolution and range before interpreting it.
"""
...
def main():
    ...
    ap.add_argument("--ckpt", type=str,
                    default="results_hybrid/colab_aligned_model.pt")
    ...
    ap.add_argument("--allow_reduced_reference", action="store_true",
                    help="Explicitly compare against a different, constant-SIR model")
    args = ap.parse_args()

    device = "cpu"
    ck = torch.load(args.ckpt, map_location=device, weights_only=False)
    targs = ck["args"]
    if targs["nn_scale"] > 0 and targs["z_res_dim"] > 0:
        if not args.allow_reduced_reference:
            ap.error("Checkpoint has neural correction; this grid omits it. Use --allow_reduced_reference only for an explicitly reduced-model comparison.")
        print("[reference] Constant-SIR reference only; not the full hybrid posterior.")
    H.N_POP = targs["n_pop"]
    ...
    model = H.HybridVAE(targs["z_res_dim"], targs["nn_scale"],
                        targs["residual_mode"],
                        full_cov=bool(targs.get("full_cov", 1)),
                        anchor=bool(targs.get("anchor", 1))).to(device)
    ...
    for r, (i, W) in enumerate(examples):
        for j, name in enumerate(H.PHYS_NAMES):
            ...
            ax.plot(axes_z[j], wm, color="#2c7fb8", lw=2, label="constant-SIR grid")
            xs = axes_z[j]
